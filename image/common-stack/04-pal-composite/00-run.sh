...
if [ -f "${CMDLINE}" ]; then
	grep -q 'vc4.tv_norm=PAL' "${CMDLINE}" || sed -i 's/$/ video=Composite-1:720x576@50ie vc4.tv_norm=PAL/' "${CMDLINE}"
	echo "cmdline.txt: video=Composite-1:720x576@50ie vc4.tv_norm=PAL appended"
fi
